...
def main():
    ...
    skl_acc = accuracy_score(np.array(actuals),np.array(guesses), normalize=True)
    """
    # report metrics
    ...
    """
    skl_report = classification_report(np.array(actuals), np.array(guesses), labels=list(range(8)),target_names=target_names)
    stats_file.write('iunno :(')
    stats_file.write(str(skl_acc))
    print(f'skl_acc {skl_acc}')
    print(skl_report)
    stats_file.write(skl_report)
    del net
